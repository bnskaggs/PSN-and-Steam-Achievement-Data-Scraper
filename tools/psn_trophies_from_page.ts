...
async function extractFromPsnProfiles(page): Promise<Row[]> {
  // Typical row pattern: .trophy or tr.trophy
  await page.waitForSelector('.trophy, tr.trophy', { timeout: 15000 }).catch(() => {});
  const rows: Row[] = await page.$$eval('.trophy, tr.trophy', (els, href) => {
    ...
    return Array.from(els).map((el: Element) => {
      ...
      const img = el.querySelector('img[src*="trophy"], img.avatar');
      const icon = (img && (img as HTMLImageElement).src) || '';
      const idAttr =
...
async function extract(url: string): Promise<Row[]> {
  ...
  try {
    ...
    const rows: Row[] = await page.evaluate((href) => {
      function num(s: string) {
        const m = s.match(/(\d+(\.\d+)?)/);
        return m ? Number(m[1]) : '';
      }
